  ...
  return (
    <main className="w-full min-h-[100svh] overflow-x-hidden bg-transparent text-[#f7f7f2] relative">
      ...
      */}
      ...
      {/* DIVIDER: Hero -> Propiedades */}
      <SectionDivider
        id="divider-propiedades"
        label="01 // CATÁLOGO DE RESIDENCIAS"
        accent="lime"
      />
      ...
      <section
        id="propiedades"
        ref={propiedadesRef}
        className="portfolio-section w-full flex flex-col justify-between"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[12px] w-full">
            {filteredProperties.map((property) => (
              <article
                ...
                className="property-card group flex flex-col rounded-[8px] p-1 bg-[#121418]/60 border border-white/5 hover:border-white/15 transition-all"
              >
                {/* Image Container with high-resolution Lightbox click handler */}
                <div
                  id={`property-image-${property.id}`}
                  onClick={() => setLightboxProperty(property)}
                  onKeyDown={(e) => {
                  ...
                  tabIndex={0}
                  role="button"
                  aria-label={`Abrir visor de alta resolución para inspeccionar detalles de ${property.title}`}
                  className="property-image-container relative group/img cursor-zoom-in overflow-hidden rounded-[6px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#bcff48]"
                >
                  <img
                    ...
                    loading="lazy"
                    className="property-img w-full h-full object-cover transition-transform duration-700 group-hover/img:scale-108"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-black/30 pointer-events-none" />

                  {/* Corner Zoom 4K Pill */}
                  <div
                    className="absolute top-3 left-3 z-10 flex items-center gap-1 px-2 py-1 rounded-[4px] bg-black/75 backdrop-blur-md border border-white/20 text-white/90 group-hover/img:text-[#bcff48] group-hover/img:border-[#bcff48]/40 transition-all text-[9px] font-mono tracking-wider uppercase shadow-md"
                    title="Clic para inspeccionar arquitectura en alta resolución"
                  >
                    <ZoomIn className="w-3 h-3 text-[#bcff48]" />
                    <span className="hidden sm:inline">Zoom 4K</span>
                  </div>

                  {/* Hover Lightbox Indicator Badge */}
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/img:opacity-100 transition-opacity duration-300 pointer-events-none bg-black/35 backdrop-blur-[1px]">
                    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/85 backdrop-blur-md border border-[#bcff48]/60 text-[10px] font-mono tracking-wider uppercase text-white shadow-2xl scale-95 group-hover/img:scale-100 transition-transform">
                  ...
                  <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-[10px] text-white pointer-events-none">
                ...
                <div
                  className="pt-3 px-1 pb-1 flex flex-col gap-1 portfolio-copy cursor-pointer"
                  ...
                  aria-label={`Ver dossier de ${property.title}`}
                >
      ...
      {/* DIVIDER: Propiedades -> Tour Interior con Dron */}
      <SectionDivider
        id="divider-tour"
        label="02 // EXPERIENCIA INMERSIVA 360°"
        accent="coral"
      />
      ...
      {/* DIVIDER: Tour Interior -> Asesoría Patrimonial */}
      <SectionDivider
        id="divider-asesoria"
        label="03 // ASESORÍA PATRIMONIAL"
        accent="lime"
      />
      ...
      {/* DIVIDER: Asesoría -> Despacho Privado & Contacto */}
      <SectionDivider
        id="divider-contacto"
        label="04 // DESPACHO PRIVADO & CONTACTO"
        accent="coral"
      />
        ...
        <PropertyDetailModal
          ...
          onScheduleVisit={(prop) => {
            ...
            setInquiryForm((prev) => ({
              ...
              notes: `Interesado en visita privada para ${prop.title} (${prop.reference}) en ${prop.location}.`,
            }));
            const contactoEl = document.getElementById('contacto');
            contactoEl?.scrollIntoView({ behavior: 'smooth' });
          }}
          onOpenLightbox={(prop) => setLightboxProperty(prop)}
        />
      )}

      {/* High-Resolution Architectural Lightbox Viewer */}
      {lightboxProperty && (
        <PropertyLightbox
          property={lightboxProperty}
          properties={PROPERTIES_DATA}
          onClose={() => setLightboxProperty(null)}
          onNavigate={(prop) => setLightboxProperty(prop)}
        />
      )}
    </main>
  );
